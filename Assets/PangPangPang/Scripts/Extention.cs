...
namespace PangPangPang
{
    public static class Extention
    {
        ...
        #region Fade
        public static void FadeOfCanvasGroup(this CanvasGroup cg, float to, float duration, Action action = null)
        {
            CoroutineRunner.Instance.RunCoroutine(Fade(cg, to, duration, action));
        }

        private static IEnumerator Fade(CanvasGroup cg, float to, float duration, Action action = null)
        {
            cg.SetActiveOfCanvasGroup(true);
            ...
            if (cg.alpha == 0)
            {
                cg.SetActiveOfCanvasGroup(false);
            }
        ...
        #region ObjectScale
        public static void ChangeScale(this Transform target, Vector3 to, float duration, bool isYoyo = false)
        {
            CoroutineRunner.Instance.RunCoroutine(ScaleTo(target, to, duration, isYoyo));
        }

        private static IEnumerator ScaleTo(Transform target, Vector3 to, float duration, bool isYoyo = false)
        {
            ...
            if (isYoyo)
            {
                CoroutineRunner.Instance.RunCoroutine(ScaleTo(target, from, duration));
            }
        }
        #endregion
